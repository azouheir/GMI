<?php		
	function PerfSSD_serv($id,$val,$total){
		$prct=($val*100)/$total;
		$prct=round($prct);
		$val=round($val,2);
		echo "
...
if (isset($_SESSION['var'])) {
	...
	$res=$con->query("SELECT SUM(cap_stock) FROM cluster JOIN serveur USING(id_cluster)");
	$row=$res->fetch_row();
    if ($row) {
	$res->data_seek(0);
	while ($row=$res->fetch_row()) {
		$somme=$row[0];
		$res1=$con->query("SELECT id_cluster,SUM(cap_stock) FROM cluster JOIN serveur USING(id_cluster) GROUP BY id_cluster");
		while ($row1=$res1->fetch_row()) {
			PerfSSD_Serv($row1[0],$row1[1],$somme);
		}
	}}
    }
	$res=$con->query("SELECT sum(s.cap_stock) FROM cluster c JOIN serveur s ON(c.id_cluster=s.id_cluster)
	...
	while ($row=$res->fetch_row()) {
		$somme=$row[0];
		$res1=$con->query("SELECT s.id_serveur,SUM(s.cap_stock) 
